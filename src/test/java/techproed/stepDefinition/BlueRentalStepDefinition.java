...
public class BlueRentalStepDefinition {
    ...
    public void kullaniciTablodaVerilenBilgileriKullanarakMapIleLoginOlur(DataTable data) {
        ...
        for (Map<String, String> w : data.asMaps()) {
            blueRentalPage.emailBox.sendKeys(w.get("e-mail"), Keys.TAB, w.get("password"), Keys.ENTER);
            ReusableMethods.bekle(3);
            Driver.getDriver().navigate().back();
            ReusableMethods.bekle(3);
        }
    ...
    @And("kullanici exceldeki {string} sayfasindaki bilgler ile giris yapip giris yapildigini test eder")
    public void kullaniciExceldekiSayfasindakiBilglerIleGirisYapipGirisYapildiginiTestEder(String sayfaIsmi) {
